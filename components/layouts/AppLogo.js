...
const AppLogo = ({ sideText, link, imgUrl = "/images/logo-she-farmers.png", imageClassName, className, children }) => {
  const router = useRouter();
  return (
    <Link href={link || router.pathname} className={`flex items-center w-max space-x-3 ${className}`}>
      <div className="">
        {imgUrl ? <img height={40} src={imgUrl} className={` !w-[200px] ${imageClassName}`} /> : null}
