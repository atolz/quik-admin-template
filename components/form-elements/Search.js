...
const Search = ({ reference, onChange, className, ...props }) => {
  return (
    <input
      style={{
      ...
      placeholder="Search"
      {...props}
      className={` h-12 px-3 py-2 text-sm text-color_text_default placeholder:text-[#97A0AF] rounded-lg bg-[#F0F3F5] border-[#BDBDBD] border focus:shadow-none focus:ring-0 focus:border-color-primary focus:outline-none ${className}`}
    ></input>
  );
};
